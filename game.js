...
class bootGame extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.audio("grow", [
      ...
      "Assets/sound/grow.mp3",
    ]);
    this.load.image("restart", "Assets/restart.png");
    this.load.image("scorepanel", "Assets/score_panel.png");
...
class playGame extends Phaser.Scene {
  ...
  create() {
    ...
    var restarButton = this.add.sprite(restartXY.x, restartXY.y, "restart");
    var scoreXY = this.getTilePosition(-0.8, 1);
    this.add.image(scoreXY.x, scoreXY.y, "scorepanel");
    this.add.image(scoreXY.x, scoreXY.y - 55, "scorelabels");
    var logo = this.add.image(450, 100, "logo");
    var gameTitle = this.add.sprite(
      game.config.width / 2,
      game.config.height - 170,
      "howtoplay"
    );
    restarButton.setInteractive();
    restarButton.on(
      "pointerdown",
      function () {
        this.scene.start("PlayGame");
